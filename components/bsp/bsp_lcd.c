...
esp_err_t bsp_lcd_init(void)
{
    ...
    gpio_pad_select_gpio(GPIO_LCD_BL);
...
esp_err_t lcd_clear_icon_area(esp_lcd_panel_t *panel, uint16_t color)
{
    ...
    uint16_t *buffer = heap_caps_malloc(160 * block_line * sizeof(uint16_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (NULL == buffer) {
        ESP_LOGE(TAG, "Memory for bitmap is not enough");
        return ESP_FAIL;
    } else {
