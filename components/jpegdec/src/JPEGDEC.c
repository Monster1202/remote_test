        ...
        for (x = 0; x < cx && bContinue && iErr == 0; x++)
        {
            ...
            if (xoff == iPitch || x == cx-1) // time to draw
            {
                ...
                if ((cx - 1 - x) < iMCUCount) // change pitch for the last set of MCUs on this row
                    iPitch = (cx - 1 - x) * mcuCX;
                //printf("time to draw jd.x:%d;jd.y:%d\n",jd.x,jd.y);
                //printf("time to draw pJPEG->iVLCSize:%d;pJPEG->iVLCOff:%d\n",pJPEG->iVLCSize,pJPEG->iVLCOff);
            }
            //printf("pJPEG.iWidth:%d;pJPEG.iHeight:%d\n",pJPEG->iWidth,pJPEG->iHeight);
            // for(int i =0;i<100;i++)
            //     printf("%d\n",*(pJPEG->pUser+i));
